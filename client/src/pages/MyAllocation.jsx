...
export default function MyAllocation(){
 ...
 return <div><PageTitle eyebrow="FACULTY VIEW" title="My Proposed Allocations" desc="Your allocation status and reasoning. Other faculty scoring details are private."/>
  ...
  <div className="space-y-3">{items.map(item=><Card className="p-5" key={item._id}><div className="flex items-start justify-between gap-3"><div><div className="text-xs font-bold text-blue-700">{item.courseId} {item.sectionId&&`• ${formatSection(item.courseYear,item.sectionId)}`}</div><h2 className="font-display text-lg font-semibold text-slate-800 mt-1">{item.courseName}</h2></div><Badge tone={item.status==='approved'?'green':(item.status==='pending'||item.status==='recommended')?'amber':'red'}>{item.statusLabel}</Badge></div><p className="mt-4 text-sm text-slate-600">{item.justification}</p>{item.status!=='approved'&&<div className="mt-3 text-xs text-slate-500">The HOD review decision is still pending or this request was not selected.</div>}</Card>)}</div>
 </div>;
}
