...
export default function Preferences(){
 ...
 const save=async()=>{try{setError('');const ranked=preferences.map((p,i)=>({courseId:p.courseId,rank:i+1}));const r=await apiRequest({method:'PUT',url:`/data/faculty/${facultyId}/preferences`,data:{preferences:ranked}});setPreferences(r.data.preferences||[]);setMessage('Preferences saved. Your ranked courses have been filed as requests for HOD review.');}catch(e){setError(e?.response?.data?.message||'Could not save preferences')}};
