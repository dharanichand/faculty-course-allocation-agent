...
import {sendAllocationDecisionEmail} from '../services/emailService.js';

const r=Router();
const dbReady=()=>mongoose.connection.readyState===1 && process.env.DATA_SOURCE==='mongodb';

async function notifyFaculty(item, decision, reason='') {
  try {
    if (!item?.facultyId) return;
    const user = await User.findOne({facultyId:String(item.facultyId)}).lean();
    if (!user?.email) return;
    await sendAllocationDecisionEmail({
      email:user.email,
      name:user.name,
      courseId:item.courseId,
      courseName:item.courseName || item.course || item.courseId,
      decision,
      reason
    });
  } catch (mailError) {
    console.error('Allocation email failed:', mailError.message);
  }
}
...
r.post('/:id/approve',auth,role('hod'),async(req,res)=>{
 try{
  ...
  if(dbReady()){
   ...
   await notifyFaculty(item,'approved');
...
r.post('/:id/reject',auth,role('hod'),async(req,res)=>{
 try{
  ...
  if(dbReady()){
   ...
   const remaining=await Allocation.find({status:{$in:['pending','recommended']}}).sort({createdAt:1}).lean();
   await notifyFaculty(item,'rejected',reason);
   return res.json({ok:true,allocation:item,remaining});
  }

  item=await decideMemoryAllocation(req.params.id,'rejected',{overrideReason:reason});
  if(!item)return res.status(404).json({message:'Allocation not found'});
  for(const other of memoryRequests){
   if(other._id!==item._id && other.courseId===item.courseId && other.sectionId===item.sectionId && other.facultyId===item.facultyId && ['pending','recommended'].includes(other.status)){
    Object.assign(other,{status:'rejected',overrideReason:reason});
   }
  }
  await notifyFaculty(item,'rejected',reason);
  return res.json({ok:true,allocation:item,remaining:await pendingAllocations()});
...
r.post('/:id/override',auth,role('hod'),async(req,res)=>{
 try{
  ...
  if(dbReady()){
   ...
   const remaining=await Allocation.find({status:{$in:['pending','recommended']}}).sort({createdAt:1}).lean();
   await notifyFaculty(item,'approved',reason);
   return res.json({ok:true,allocation:item,remaining});
  }
  ...
  if(!item)return res.status(404).json({message:'Allocation not found'});
  await notifyFaculty(item,'approved',req.body.reason||'Override by HOD');
  return res.json({ok:true,allocation:item,remaining:await pendingAllocations()});
 }catch(e){res.status(400).json({message:e.message})}
});
