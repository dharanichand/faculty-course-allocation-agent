...
const schema=new mongoose.Schema({courseId:{type:String,unique:true},courseCode:String,courseName:String,department:String,academicYear:{type:String,default:''},semester:{type:String,default:''},credits:Number,theoryHours:Number,labHours:Number,tutorialHours:Number,requiredExpertise:[String],requiredQualification:[String],sections:[Object],studentStrength:Number,status:{type:String,default:'open'},
 program:String,year:String,courseType:String,shortName:String,sectionCount:Number,instructorsPerSection:{type:Number,default:1},hoursPerSection:Number,
 // The department's own "Lead Faculty" sheet names one coordinating faculty
 // member per course. officialLeadFacultyId is set only when that name could
 // be confidently matched to a real facultyId; officialLeadFacultyName always
 // keeps the raw sheet text so an unmatched name is still visible for review.
 officialLeadFacultyId:{type:String,default:null},officialLeadFacultyName:{type:String,default:''}},{timestamps:true});
